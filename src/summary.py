#DATE:09/01/2024
#Set up a new Python virtual environment for the backend development.
#Install required libraries such as Flask, Sumy, and NLTK.
...
#make bullet points
    ...
    def create_bullet_points(text, format_choice):
    # Load spaCy model
        if format_choice == "bullets":
            ...
            combined_sentences = [" ".join([sent.text.strip() for sent in sentences[i:i + 3]]) for i in
                                range(0, len(sentences), 3)]
...
if __name__ == "__main__":
    ...
    docx_path = "312079761-Telugu-Kavithalu.pdf"
    extracted_text = processor.extract_text_from_pdf(docx_path)
